...
def parse_header(header_path: Path) -> Optional[ActorInfo]:
    ...
    for match in field_pattern.finditer(struct_body):
        ...
        # Track special fields (only first occurrence)
        if (field_name == 'skelAnime' or type_str == 'SkelAnime') and not actor_info.has_skel_anime:
            actor_info.skel_anime_offset = offset
            actor_info.has_skel_anime = True
        elif field_name == 'jointTable' and actor_info.joint_table_size == 0:
        ...
        elif field_name == 'morphTable' and actor_info.morph_table_size == 0:
            base, size = parse_array_size(type_str)
            if size < 0:
                # Try to resolve named constant
                match = re.search(r'\[(\w+)\]', type_str)
        ...
        elif field_info.is_action_func:
...
def generate_c_code(actors: List[ActorInfo]) -> str:
    ...
    lines.append("// Field descriptors for syncable state variables")
    for actor in actors:
    ...
    for actor in actors:
        ...
        lines.append(f"    {{ {actor.actor_id}, 0x{actor.struct_size:X}, 0x{actor.skel_anime_offset:X}, "
                    f"0x{actor.action_func_offset:X}, {actor.joint_table_size}, {actor.morph_table_size}, "
                    f"{func_table}, {func_count}, {state_fields}, {state_count} }},")
    lines.append("    { -1, 0, 0, 0, 0, 0, NULL, 0, NULL, 0 }  // Terminator")
    lines.append("};")
    lines.append("")
