...
void Anchor::HandlePacket_PlayerUpdate(nlohmann::json payload) {
    try {
        ...
        // Set up interpolation target for smooth movement
        PosRot newPosRot = payload["posRot"].get<PosRot>();

        // Just update the target - DummyPlayer_Update will lerp toward it
        // Teleport detection is handled there by checking distance to target
